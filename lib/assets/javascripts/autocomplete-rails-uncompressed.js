      ...
      jQuery(e).autocomplete({
        source: function( request, response ) {
          jQuery.getJSON( jQuery(e).attr('data-autocomplete'), {
            ...
            if (jQuery(e).data('root')) {
              var root = jQuery(e).data('root');
              if (arguments[0] && arguments[0][root]) {
                suggestions = arguments[0][root];
              }
            }

            suggestions = _.uniq(_.flatten(_.map(suggestions, function(suggestion, i){
              return split(suggestion.label);
            })));
